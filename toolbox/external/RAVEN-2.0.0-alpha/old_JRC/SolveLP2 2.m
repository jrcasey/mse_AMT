function [solution,hsSolOut,res]=SolveLP2(model,minFlux,params,hsSol)
% % % solveLP2
...
% % %         y       dual cost or shadow price or dual price
% % %         stat    exit flag 
...
% % %            res  Direct output from MOSEK (Solution structure without formatting)
% % %
% % %   Usage: [solution hsSolOut]=solveLP2(model,minFlux,params,hsSol)
% % %
% % %   Version2 for RAVEN function solveLP(),RASMUS, 2013-07-16
% % %   added by Partho on 18/09/2014 ;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if isfield(res.sol,'bas')
    ...
    solution.prosta=res.sol.bas.prosta; %% problem status
    solution.skc=res.sol.bas.skc; %% Constraint status keys
    solution.skx=res.sol.bas.skx; %% Variable status keys
    solution.xc =res.sol.bas.xc; %% Constraint activities
   
    
    solution.x=res.sol.bas.xx; %% primal solutions
    solution.y =res.sol.bas.y.*(-1); %% dual solutions
    
    solution.slc =res.sol.bas.slc; %% Dual variables corresponding to lower constraint bounds.
    solution.suc =res.sol.bas.suc; %% Dual variables corresponding to upper constraint bounds.
    solution.slx =res.sol.bas.slx; %% Dual variables corresponding to lower variable bounds.
    solution.sux =res.sol.bas.sux; %% Dual variables corresponding to upper variable bounds.
    %solution.snx =res.sol.bas.snx; %% Dual variables corresponding to the conic constraints.

    if minFlux<=1;
        hsSolOut=res.sol.bas;
    end
    solution.f=res.sol.bas.pobjval; %% objective function
else
...
if minFlux~=0
    ...
    switch minFlux
        ...
        case 2
%         if size(model.b,2)==1
%             qsol=solveQP(model,model.rxns,z neros(numel(model.lb),1));
