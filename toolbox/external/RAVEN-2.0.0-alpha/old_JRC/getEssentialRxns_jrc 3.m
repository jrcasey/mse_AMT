function [essentialRxns, essentialRxnsIndexes]=getEssentialRxns_jrc(model,ignoreRxns)
% getEssentialRxns
...
rxnsToCheck=setdiff(model.rxns(abs(sol.x)~=0),ignoreRxns);
...
while 1
    ...
    rxnsToCheck=intersect(rxnsToCheck,model.rxns(abs(sol.x)~=0));
