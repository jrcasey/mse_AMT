...
YI=(1:NConsume)';
barh(YI,abs(v(XIMax(1:NConsume))));
% title(['Consumers (top) and Producers of ' metAbbr],'FontSize',16)

if NConsume==0
    set(h1,'YTickLabel','NONE','FontSize',18);
else
    for n=1:NConsume
        %if strcmp(model.rxns(XIMax(n)).directionality,'forward')
    ...
    set(h1,'YTickLabel',YTickLabelMax,'FontSize',18);
    ylim([0.5 NConsume+0.5])
end
xlabel(['Consumer of ' metAbbr],'FontSize',18)

h2=subplot(2,1,2);
NProduce=min([NProduce,N]);
YI=(1:NProduce)';
barh(YI,abs(v(XIMin(1:NProduce))));
if NProduce==0
    set(h1,'YTickLabel','NONE','FontSize',18);
else
    for n=1:NProduce
    ...
    set(h2,'YTickLabel',YTickLabelMin,'FontSize',18);
    ylim([0.5 NProduce+0.5])
end
xlabel(['Producer of ' metAbbr],'FontSize',18)
